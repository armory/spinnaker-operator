...
import (
	...
	"github.com/armory/spinnaker-operator/pkg/api/deploy/spindeploy/changedetectortest"
...
func TestIsSpinnakerUpToDate_TestExposeConfigChangedPort(t *testing.T) {
	...
	spinSvc := test.ManifestFileToSpinService("testdata/spinsvc_expose.yml", t)
	spinSvc.GetExposeConfig().Service.PublicPort = 80
...
func TestIsSpinnakerUpToDate_UpToDateWithOverrides(t *testing.T) {
	...
	ch := changedetectortest.SetupChangeDetector(&ChangeDetectorGenerator{}, t, deckSvc, gateSvc)
	s := `
apiVersion: spinnaker.io/v1alpha2
kind: SpinnakerService
metadata:
...
func TestIsSpinnakerUpToDate_OverrideBaseUrlAdded(t *testing.T) {
	...
	spinSvc := test.ManifestFileToSpinService("testdata/spinsvc_expose.yml", t)
	spinSvc.GetExposeConfig().Service.PublicPort = 0
	err := spinSvc.GetSpinnakerConfig().SetHalConfigProp(util.GateOverrideBaseUrlProp, "https://acme-api.com")
	assert.Nil(t, err)
...
func TestIsSpinnakerUpToDate_NoAnnotations(t *testing.T) {
	...
	ch := changedetectortest.SetupChangeDetector(&ChangeDetectorGenerator{}, t, deckSvc, gateSvc)
	spinSvc := test.ManifestFileToSpinService("testdata/spinsvc_expose.yml", t)
	spinSvc.GetExposeConfig().Service.Annotations = nil
